...
static CK_RV wosign_login();
static CK_RV wosign_logout(void);
static CK_RV wosign_find_key_pai();
...
int wosign_config_init(wosign_sign_config *config)
{
	...
	_sign_info.mechanism = {config->algorithm, NULL, 0};
	return 0;
}
...
static CK_RV wosign_init(void) {
	...
	_sign_info.config.pin = NULL;
	_sign_info.config.container = NULL;
	_sign_info.config.id = NULL;
	_sign_info.config.algorithm = 0;
	_sign_info.mechanism = {0, NULL, 0};
...
int wosign_sign(const char* data, const size_t len, char** signature, size_t* sig_len)
{
	...
	deepin_message(LOG_DEBUG,"初始化环境[成功]\n");
	...
	deepin_message(LOG_DEBUG,"登录[成功]\n");
	...
	deepin_message(LOG_DEBUG,"查找密钥[成功]\n");
	...
	deepin_message(LOG_DEBUG,"签名[成功]\n");
	...
	deepin_message(LOG_DEBUG,"退出登录[成功]\n");
